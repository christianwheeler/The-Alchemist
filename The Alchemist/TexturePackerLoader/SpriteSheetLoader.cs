namespace TexturePackerLoader
{
    ...
    using The_Alchemist.Properties;
    ...
    public class SpriteSheetLoader
    {
        ...
        public SpriteSheet Load(string imageResource)
        {
            ...
            var dataFileLines = this.ReadDataFile(imageResource);
                    ...
                    select row.Split(';'))
            {
                ...
                var pX = float.Parse(cols[8], CultureInfo.InvariantCulture.NumberFormat);
                var pY = float.Parse(cols[9], CultureInfo.InvariantCulture.NumberFormat);

                var pivotPoint = new Vector2(pX, pY);
        ...
        private string[] ReadDataFile(string dataFile) 
        {
            return File.ReadAllLines("Content/" + dataFile + ".txt");
        }
#endif
    }
}